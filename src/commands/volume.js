    ...
    try {
      if (isNaN(newVolume)) {
        msg.channel.send(
          util
            .embed()
            .setDescription(`🔉 | Current volume \`${music.volume}\`.`)
        );
      } else {
        ...
        if (newVolume < 0 || newVolume > 150)
          return msg.channel.send(
            util
              .embed()
              .setDescription("❌ | You can only set the volume from 0 to 150.")
          );

        await music.setVolume(newVolume);
        msg.channel.send(
          util.embed().setDescription(`🔉 | Volume set to \`${music.volume}\`.`)
        );
      }
    } catch (e) {
